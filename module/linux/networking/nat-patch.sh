...
APPLY_NFT_BACKUP=""
APPLY_IPT_BACKUP=""
APPLY_FORWARDING_CHANGED=0
APPLY_MANIFEST_LINES=0

cleanup_apply_temp() {
    [ -n "$APPLY_NFT_BACKUP" ] && rm -f "$APPLY_NFT_BACKUP"
    [ -n "$APPLY_IPT_BACKUP" ] && rm -f "$APPLY_IPT_BACKUP"
    APPLY_NFT_BACKUP=""
    APPLY_IPT_BACKUP=""
}
...
ensure_nft_include() {
    ...
    # Validate the candidate before replacing the live configuration file.
...
persist_nft() {
    ...
    if ! nft list table ip nat_tool > "$tmp_file" 2>/dev/null; then
        rm -f "$tmp_file"; log_error "Could not dump nftables table for persistence."; return 1
    fi
    ...
    if command -v systemctl >/dev/null 2>&1; then
        if systemctl enable nftables >/dev/null 2>&1; then
            ...
            log_error "Could not enable nftables.service automatically."; return 1
        fi
    else
        log_error "systemctl is unavailable; cannot enable nftables persistence service."; return 1
    fi
    return 0
}
...
EOF
    ...
    if command -v systemctl >/dev/null 2>&1 && systemctl daemon-reload >/dev/null 2>&1 && systemctl enable nat-tool-restore >/dev/null 2>&1; then
        log_success "nat-tool-restore.service enabled (restores rules at boot)."
    else
        log_error "Could not enable restore unit automatically."; return 1
    fi
    return 0
}
...
# ----------------------------- Apply rules ------------------------------------
nft_delete_nat_tool_if_present() {
    if nft list table ip nat_tool >/dev/null 2>&1; then
        nft delete table ip nat_tool >/dev/null 2>&1
    else
        return 0
    fi
}
...
ipt_delete_if_exists() {
    local table_args=() chain
    if [ "$1" = "-t" ]; then table_args=("$1" "$2"); shift 2; fi
    chain="$1"; shift
    if iptables "${table_args[@]}" -C "$chain" "$@" >/dev/null 2>&1; then
        iptables "${table_args[@]}" -D "$chain" "$@" || return 1
    fi
    return 0
}

apply_ipt() {
    local wan="$1" lan="$2" dmz="$3" ifc subnet l d
    mkdir -p "$BACKUP_DIR" || return 1
    APPLY_IPT_BACKUP=$(mktemp "$BACKUP_DIR/nat-tool-ipt-apply.XXXXXX") || return 1
    if ! iptables-save > "$APPLY_IPT_BACKUP"; then log_error "Could not snapshot current iptables rules."; cleanup_apply_temp; return 1; fi
    for ifc in $lan $dmz; do
        subnet=$(subnet_of "$ifc")
        if [ -z "$subnet" ]; then log_error "Interface $ifc has no IPv4 address - cannot build MASQUERADE rule."; return 1; fi
        ipt_delete_if_exists -t nat POSTROUTING -o "$wan" -s "$subnet" -j MASQUERADE || return 1
    ...
    if [ -z "$lan$dmz" ]; then
        ipt_delete_if_exists -t nat POSTROUTING -o "$wan" -j MASQUERADE || return 1
        iptables -t nat -A POSTROUTING -o "$wan" -j MASQUERADE || return 1
        record ipt_rule "-t nat -D POSTROUTING -o $wan -j MASQUERADE"
    fi
    if [ -n "$lan" ]; then
        for l in $lan; do
            ipt_delete_if_exists FORWARD -i "$l" -o "$wan" -j ACCEPT || return 1
            iptables -A FORWARD -i "$l" -o "$wan" -j ACCEPT || return 1
            record ipt_rule "-D FORWARD -i $l -o $wan -j ACCEPT"
            ipt_delete_if_exists FORWARD -i "$wan" -o "$l" -m state --state RELATED,ESTABLISHED -j ACCEPT || return 1
            iptables -A FORWARD -i "$wan" -o "$l" -m state --state RELATED,ESTABLISHED -j ACCEPT || return 1
            record ipt_rule "-D FORWARD -i $wan -o $l -m state --state RELATED,ESTABLISHED -j ACCEPT"
        done
    fi
    if [ -n "$dmz" ]; then
        for d in $dmz; do
            ipt_delete_if_exists FORWARD -i "$d" -o "$wan" -j ACCEPT || return 1
            iptables -A FORWARD -i "$d" -o "$wan" -j ACCEPT || return 1
            record ipt_rule "-D FORWARD -i $d -o $wan -j ACCEPT"
            if [ -n "$lan" ]; then
                for l in $lan; do
                    ipt_delete_if_exists FORWARD -i "$l" -o "$d" -j DROP || return 1
                    iptables -A FORWARD -i "$l" -o "$d" -j DROP || return 1
                    record ipt_rule "-D FORWARD -i $l -o $d -j DROP"
                    ipt_delete_if_exists FORWARD -i "$d" -o "$l" -j DROP || return 1
                    iptables -A FORWARD -i "$d" -o "$l" -j DROP || return 1
...
restore_forwarding_after_failed_apply() {
    if [ "$APPLY_FORWARDING_CHANGED" -eq 1 ]; then
        local previous
        previous=$(tac "$MANIFEST" 2>/dev/null | awk -F'\|' '$1=="sysctl_forward" {print $2; exit}')
        if [ -n "$previous" ]; then
            if sysctl -w net.ipv4.ip_forward="$previous" >/dev/null 2>&1; then log_success "Restored ip_forward=$previous after failed apply."; else log_error "Could not restore ip_forward=$previous after failed apply."; fi
        fi
        APPLY_FORWARDING_CHANGED=0
    fi
}
...
main_wizard() {
    ...
    APPLY_MANIFEST_LINES=$(wc -l < "$MANIFEST" 2>/dev/null || echo 0)
    ...
    if [ $apply_rc -eq 0 ]; then
        ...
        log_error "Rule application failed - restoring pre-apply state..."
        if [ "$BACKEND" = "ipt" ] && [ -n "$APPLY_IPT_BACKUP" ] && [ -s "$APPLY_IPT_BACKUP" ]; then
            if iptables-restore < "$APPLY_IPT_BACKUP"; then log_success "Restored previous iptables rules."; else log_error "CRITICAL: failed to restore previous iptables rules."; fi
        fi
        cleanup_apply_temp
        restore_forwarding_after_failed_apply
        if [ -f "$MANIFEST" ]; then
            head -n "$APPLY_MANIFEST_LINES" "$MANIFEST" > "${MANIFEST}.tmp" && mv -f "${MANIFEST}.tmp" "$MANIFEST"
        fi
        log_error "Rule application failed."
        return 1
    fi
}
